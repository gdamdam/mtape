 ...
 * guard the vendoring — that the module graph resolves under mtape's tsconfig
...
describe('vendored mbus-client', () => {
  ...
  it('builds outbound frames that parse back as JSON', () => {
    ...
    expect(JSON.parse(outbound.announce('mtape'))).toEqual({
      type: 'mbus/announce',
      name: 'mtape',
    })
  })
  ...
  it('parses a welcome frame and ignores non-mbus traffic', () => {
    const welcome = JSON.stringify({
      ...
      clientId: 'c1',
      mbus: MBUS_VERSION,
      sources: [{ sourceId: 's1', name: 'mtape', clientId: 'c2' }],
    })
